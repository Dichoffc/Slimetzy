...
const jadibot = async (slimecode, m, from) => {
if (Object.keys(client).includes(from)) {
    return slimecode.sendMessage(from, { text: "Kamu sudah jadibot sebelumnya!" }, {quoted: m})
  }
const { state, saveCreds } = await useMultiFileAuthState(`./data/slimecode-clone/${m.sender.split("@")[0]}`)
try {
...
require('../../slimecode')(client[from], m, chatUpdate, mek, store)
}
} catch (err) {
console.log(err)
}
...
connectToWhatsApp();
  ...
  slimecode.sendMessage(from, { text: `Bot Clone Terhubung...` }, {quoted: m});
  await sleep(3000)
...
async function stopjadibot(slimecode, m, from) {
  if (!Object.keys(client).includes(from)) {
    return slimecode.sendMessage(from, { text: `Kamu tidak ada di list jadibot!` }, {quoted: m})
  }
  try {
    client[from].end("Stop")
  } catch {}
  delete client[from]
  rimraf.sync(`./data/slimecode-clone/${from.split("@")[0]}`)
}

async function listjadibot(slimecode, m) {
  ...
  return slimecode.sendMessage(from, { text: text.trim(), mentions, }, {quoted: m})
}
